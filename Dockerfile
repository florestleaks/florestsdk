...
ENV VIRTUAL_ENV /opt/florestsdk-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/florestsdk/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/florestsdk/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/florestsdk/ && touch src/florestsdk/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/florestsdk/
...
ENTRYPOINT ["/opt/florestsdk-env/bin/poe"]
CMD ["api"]
